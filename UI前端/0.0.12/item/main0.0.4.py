...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowFlag(QtCore.Qt.FramelessWindowHint)
        self.setAttribute(QtCore.Qt.WA_TranslucentBackground)
        self.ui.ex_class2_1.clicked.connect(self.request_question)
        self.data = None
    ...
    def request_question(self):
        ...
        option = response.json()[MainWindow.IP]["answer"]     # 数据库中存储的答案选项
    ...
    def judge_answer(self, answer):
        ...
        if sender.text() == answer:
            sender.setStyleSheet('background-color: green;border-radius: 15px')   # 答案正确时按钮变绿
        else:
            sender.setStyleSheet('background-color: red;border-radius: 15px')  # 答案错误时按钮变红
            url = "http://172.29.1.106:8000/question/"
            myParams = {"user_id": "3","question_id": self.data.json()[MainWindow.IP]["id"]}  # 发送科目名称

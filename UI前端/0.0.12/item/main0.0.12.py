# 更新程序猿：蛋蛋
# 更新时间：2024年1月3日17:31:03
# 内容：把答题卡bug修复了，做完习题页面也完善了，上传了app图标，完成了设置里的关于和帮助，暂无新bug。
...
class LoginWindow(QMainWindow):
    ...
    def go_to_main(self):
        ...
        if self.account_data["success"]:
            ...
            custom_icon = QIcon('研题.png')  # 设置标题栏图像
            message_box.setWindowIcon(custom_icon)  # 设置标题栏图像
    ...
    # 用户注册
    def register(self):
        name = self.ui.account_2.text()
        account = self.ui.PhoneNumber.text()
        password = self.ui.password_2.text()
        confirm = self.ui.password_confirmed.text()
        if password == confirm:
            url = "http://172.29.1.106:8000/register/"
            params = {"phone_number": account, "password": password, "user_name": name}
            res = requests.post(url=url, data=params)
            print(res.json())
            self.account_data = res.json()[0]
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, login):
        ...
        for idx1, btn in enumerate([self.ui.lx_class1_2, self.ui.lx_class2_2, self.ui.lx_class3_2,
                                   self.ui.lx_class4_2, self.ui.lx_class5_2, self.ui.lx_class6_2,
                                   self.ui.lx_class7_2]):
            btn.clicked.connect(lambda _, idx=idx1: self.request_question(classes[idx]))
        ...
        self.ui.exercise_next_3.clicked.connect(lambda: self.is_set("practice"))
        self.ui.exercise_next_2.clicked.connect(lambda: self.is_set("wrong"))
        ...
        self.ui.Bt_error.clicked.connect(lambda: self.wrong_ex("practice"))
        self.ui.Bt_error_2.clicked.connect(lambda: self.wrong_ex("wrong"))
        self.been_choose = "no"
        self.been_choose2 = None
        ...
        for i in range(1, 26):
            button = getattr(self.ui, f'exercise_Q{i}_3')
            # 断开之前按钮的连接
            if i in previous_connections:
                previous_connections[i].disconnect()
            # 连接新按钮并存储连接
            previous_connections[i] = button.clicked.connect(lambda _, idx=i: self.select_question(idx))

        self.selected_options2 = [None] * num_questions  # 初始化选项数组
        # 用户点击错题答题卡选择某题目
        previous_connections2 = {}
        for i in range(1, 26):
            button2 = getattr(self.ui, f'exercise_Q{i}_2')
            # 断开之前按钮的连接
            if i in previous_connections2:
                previous_connections2[i].disconnect()
            # 连接新按钮并存储连接
            previous_connections2[i] = button2.clicked.connect(lambda _, idx=i: self.select_question2(idx))
    ...
    def sign_in(self):
        ...
        self.ui.bt_sign_in.setText("已签到")
    ...
    def select_question(self, question_id):
        ...
        if self.selected_options[question_id-1] is None:
            self.practice_id = question_id - 1
            self.been_choose = "no"
            for i in {'A', 'B', 'C', 'D'}:
                getattr(self.ui, f'Bt_{i}_ex_3').setEnabled(True)
            self.set_question(None)
        else:
            self.been_choose = "yes"
            self.practice_id = question_id - 1
            self.set_question("had")
    # 用户点击错题答题卡按钮跳转题目现场
    def select_question2(self, question_id):
        self.ui.stackedWidget_3.setCurrentIndex(1)
        if self.selected_options2[question_id-1] is None:
            self.wrong_id = question_id - 1
            self.been_choose2 = "no"
            for i in {'A', 'B', 'C', 'D'}:
                getattr(self.ui, f'Bt_{i}_ex_2').setEnabled(True)
            self.set_wrong(None)
        else:
            self.been_choose2 = "yes"
            self.wrong_id = question_id - 1
            self.set_wrong("had_wrong")
    ...
    def request_question(self, subject_name):
        ...
        self.practice_id = 0
        for i in (0, 25):
            self.selected_options[i] = None
        self.set_question(None)
        self.update_answer_sheet.change_button('exercise')

    def is_set(self, kind):
        if kind == "practice" and self.practice_id != 25:
            if self.selected_options[self.practice_id] is None:
                self.been_choose = "no"
                for i in {'A', 'B', 'C', 'D'}:
                    getattr(self.ui, f'Bt_{i}_ex_3').setEnabled(True)
                self.set_question(None)
        ...
        # 题号个数范围内
        elif self.practice_id != 25 and self.wrong_id != 25 and len(self.wrong_data.json()) != self.wrong_id:
            print(len(self.wrong_data.json()))
            if self.selected_options2[self.wrong_id] is None:
                self.been_choose2 = "no"
                for i in {'A', 'B', 'C', 'D'}:
                    getattr(self.ui, f'Bt_{i}_ex_2').setEnabled(True)
                self.set_wrong(None)
            ...
            if kind == "practice":
                ...
                self.finish_page.wrong_finish(self.right_number, self.wrong_number, len(self.wrong_data.json()))
                self.ui.stackedWidget_3.setCurrentIndex(3)
                self.wrong_id = 0
                print("已无错题")
    def set_question(self, kind):
        ...
        self.set_practice_sheet()
        if kind == "had":
            for i in {'A', 'B', 'C', 'D'}:
                # 用户已经答过此题，禁用按钮再次选择
                getattr(self.ui, f'Bt_{i}_ex_3').setEnabled(False)
            shift = {
                'A': 1, 'B': 2, 'C': 3, 'D': 4
            }
            print("selected_options[self.practice_id] = ", self.selected_options[self.practice_id])
            if self.selected_options[self.practice_id] in shift:
                ...
                self.ui.sigle_or_more_3.setText("题号：" + str(self.practice_id))
                return 0
    ...
    def judge_answer(self, answer, btn):
        ...
        if btn in option_mapping and btn < 5:
            self.selected_options[self.practice_id-1] = option_mapping[btn]
            print("题号:", self.practice_id, "用户选择：", self.selected_options[self.practice_id-1], "答案", answer)
        elif btn in option_mapping and btn > 4:
            self.selected_options2[self.wrong_id-1] = option_mapping[btn]
        for button, btn_value in self.button_answers.items():
            if btn_value == btn:
                if button.text() != answer:
                    button.setStyleSheet('background-color: rgb(255, 105, 105);border-radius: 15px')
                    # 答错题加入错题集，改变答题卡
                    if btn < 5 and self.been_choose == "no":
                        self.wrong_number = self.wrong_number + 1  # 用户答错题数量＋1
                        self.update_answer_sheet.no_button('exercise', self.practice_id)
                        self.put_question()
                    elif btn > 4:
                        print("wrong_id = ", self.wrong_id)
                        self.update_answer_sheet.no_button('wrong', self.wrong_id)
                else:
                    button.setStyleSheet('background-color: rgb(170, 255, 161);border-radius: 15px')
    ...
    def put_question(self):
        url = "http://172.29.1.106:8000/question/"
        print("practice_id = ",self.practice_id)
        print("user_id",self.login.account_data["id"])
        print("question_id", self.practice_data.json()[self.practice_id-1]["id"])
        myParams = {"user_id": self.login.account_data["id"], "question_id": self.practice_data.json()[self.practice_id-1]["id"]}  # 发送用户ID以及科目ID
    ...
    def wrong_ex(self, btn):
        ...
        if btn == "practice" and self.practice_id != 0:
            myParams = {"question_id": self.practice_data.json()[self.practice_id]["id"]}
        elif self.wrong_id != 0:
            myParams = {"question_id": self.wrong_data.json()[self.wrong_id]["id"]}
        else:
            print("错误，这一步不应该发生")
            return 0
        res = requests.put(url=url, data=myParams)
        if res.text == "\"OK\"":
            ...
            custom_icon = QIcon('研题.png')  # 设置标题栏图像
            message_box.setWindowIcon(custom_icon)  # 设置标题栏图像
            message_box.setIcon(QMessageBox.Information)  # 设置图标为感叹号
            message_box.setStandardButtons(QMessageBox.Ok)  # 设置按钮为确定
            # 显示消息框
            message_box.exec()
    ...
    def request_wrong(self, subject_name):
        ...
        self.wrong_id = 0
        for i in (0, 25):
            self.selected_options2[i] = None
        if len(self.wrong_data.json()):
            self.set_wrong(None)
            self.update_answer_sheet.change_button('wrong')     # 把错题答题卡初始成灰色
            self.ui.stackedWidget_3.setCurrentIndex(1)
        else:
            message_box = QMessageBox()
            # 设置消息框的属性
    ...
    def set_wrong(self, kind):
        ...
        self.set_wrong_sheet()
        if kind == "had_wrong":
            for i in {'A', 'B', 'C', 'D'}:
                # 用户已经答过此题，禁用按钮再次选择
                getattr(self.ui, f'Bt_{i}_ex_2').setEnabled(False)
            shift = {
                'A': 5, 'B': 6, 'C': 7, 'D': 8
            }
            if self.selected_options2[self.wrong_id] in shift:
                btn = shift[self.selected_options2[self.wrong_id]]
                self.wrong_id += 1
                print("btn = ", btn)
                self.judge_answer(answer, btn)
                self.ui.sigle_or_more_2.setText("题号：" + str(self.wrong_id))
                return 0
    ...
    def set_information(self):
        self.login.account_data = self.set_inform.set_information(self.login.account_data)
    ...
    # 设置答题卡界面信息
    def set_practice_sheet(self):
        information = str(self.login.account_data["user_name"])+"\n"+"Hello!"
        self.ui.text_info_3.setText(information)
        current = "当前题目："+"\n"+str(self.practice_id+1)+"/25"
        self.ui.exercise_current_3.setText(current)
    def set_wrong_sheet(self):
        information = str(self.login.account_data["user_name"]) + "\n" + "Hello!"
        self.ui.text_info_2.setText(information)
        current = "当前题目：" + "\n" + str(self.wrong_id + 1) + "/" + str(len(self.wrong_data.json()))
        self.ui.exercise_current_2.setText(current)
